...
function SignInForm({ dm }: { dm: boolean }) {
  ...
  return (
    <div style={{
      ...
      {/* The mark, per domain. On draftmasters.net this is the shipped
          wordmark rather than a re-set of it — its ground is black rather than
          transparent, so it rides on `screen` and only the brass survives. */}
      <div style={{ textAlign: "center", marginBottom: 40 }}>
        {dm ? (
          ...
          <img
            ...
            alt="DraftMasters"
            style={{ height: 46, width: "auto", display: "block", margin: "0 auto 10px", mixBlendMode: "screen" }}
          />
        ) : (
          <>
        ...
        <p style={{ margin: "6px 0 0", color: "#555", fontSize: 11, letterSpacing: "0.28em", textTransform: "uppercase" }}>
          {dm ? "TRAVEL THE MULTIVERSE AND DRAFT A TEAM" : "A GATHERING OF LEGENDS"}
        </p>
      </div>
      ...
      <div style={{ width: "100%", maxWidth: 380 }}>
        ...
        <div style={{ display: "flex", borderBottom: "1px solid #2a2a2a", marginBottom: 24 }}>
          {(["signin", "register"] as const).map(t => (
            <button key={t} onClick={() => { setTab(t); setError(""); }} style={{
              ...
              {t === "signin"
                ? (dm ? "SIGN IN" : "ENTER THE HALL")
                : (dm ? "CREATE ACCOUNT" : "JOIN THE RANKS")}
            </button>
          ))}
        </div>
          ...
          {tab === "signin" ? (
            <form onSubmit={handleSignIn} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <button type="submit" disabled={loading} style={{
                ...
                {loading ? "SIGNING IN…" : dm ? "SIGN IN" : "ENTER THE HALL"}
              </button>
            ...
            <form onSubmit={handleRegister} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <button type="submit" disabled={loading} style={{
                ...
                {loading ? "CREATING…" : dm ? "CREATE ACCOUNT" : "JOIN THE RANKS"}
              </button>
            </form>
          ) : (
        ...
        <p style={{ textAlign: "center", marginTop: 20, fontSize: 11, color: "#333" }}>
          {dm ? "No ads. No tracking. Just the draft." : "🔥 No ads. No tracking. Just legends."}
        </p>
      </div>
 ...
 */
export default function SignInClient({ dm }: { dm: boolean }) {
  return (
