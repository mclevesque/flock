...
async function ensureTable() {
  ...
  await sql`ALTER TABLE outbreak_runs ADD COLUMN IF NOT EXISTS player_count INTEGER DEFAULT 1`;
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const {
      userId, username, difficulty, kills, level, streakTier, timeSurvived,
      gold, damageDealt, survived, weapons, passives, deathCause, dmgLog,
      upgradeCount, playerCount,
    } = body;

    await sql`
      INSERT INTO outbreak_runs (
        user_id, username, kills, level, streak_tier, time_survived, gold,
        damage_dealt, survived, weapons, passives, death_cause, dmg_log,
        difficulty, upgrade_count, player_count
      ) VALUES (
        ...
        ${difficulty || 2}, ${upgradeCount || 0}, ${playerCount || 1}
      )
    `;
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [runs, totals, perUser, best, leaderboard, duosLeaderboard] = await Promise.all([
      ...
      // Solo leaderboard — best kill run per user per difficulty (solo runs only)
      sql`SELECT DISTINCT ON (username, difficulty)
            username, difficulty, kills::int as kills, damage_dealt::int as damage_dealt,
            upgrade_count::int as upgrade_count, survived, time_survived::int as time_survived,
            created_at, 0 AS items_used
          FROM outbreak_runs
          WHERE player_count = 1 OR player_count IS NULL
          ORDER BY username, difficulty, kills::int DESC`,
      // Duos leaderboard — best kill run per user per difficulty (2+ player runs)
      sql`SELECT DISTINCT ON (username, difficulty)
            username, difficulty, kills::int as kills, damage_dealt::int as damage_dealt,
            upgrade_count::int as upgrade_count, survived, time_survived::int as time_survived,
            created_at, player_count::int as player_count
          FROM outbreak_runs
          WHERE player_count >= 2
          ORDER BY username, difficulty, kills::int DESC`,
    ]);

    return NextResponse.json({ runs, totals: totals[0], perUser, best: best[0], leaderboard, duosLeaderboard });
  } catch (e) {
