...
type LobbyMsg = LobbyStateMsg | LobbyJoinMsg | LobbyLeaveMsg | GameStartMsg | HostTransferMsg;
...
export default function OutbreakLobby({ userId, username, avatarUrl, onPlay, onSolo, initialRoom }: Props) {
  ...
  // If initialRoom provided, auto-join as guest
  useEffect(() => {
  ...
  const broadcastState = useCallback((ws: typeof wsRef.current, playerList: LobbyPlayer[], started = false) => {
    if (!ws) return;
    ws.send(JSON.stringify({ type: "lobby_state", players: playerList, started }));
  }, []);
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      ...
      const myEntry: LobbyPlayer = { userId, username, avatarUrl, isHost };
      ...
      ws!.onmessage = (e: MessageEvent) => {
        try {
          ...
          if (msg.type === "lobby_join") {
            ...
            if (incoming.userId === userId) return; // ignore own echo
            setPlayers(prev => {
              ...
              // If I'm host, re-broadcast full state so new joiner gets everyone
              if (isHost) {
                broadcastState(wsRef.current, updated, gameStartedRef.current);
              }
              return updated;
            });
          }
    ...
    return () => {
      if (ws) {
        ws.send(JSON.stringify({ type: "lobby_leave", userId }));
        ws.close();
      }
      wsRef.current = null;
    };
  // We intentionally only reconnect when mode/roomCode changes
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [mode, roomCode]);
  ...
  const startGame = () => {
    wsRef.current?.send(JSON.stringify({ type: "game_start", roomCode }));
    onPlay(roomCode);
  };
  ...
  // When in lobby as host, ensure self is in list
    ...
    return (
      <div style={bg}>
        ...
        <div style={{ position: "relative", zIndex: 1, textAlign: "center", width: "100%", maxWidth: 460, padding: "0 20px" }}>
          ...
          <h1 style={{ color: "#c084fc", fontSize: 32, fontWeight: 900, letterSpacing: 6, margin: "0 0 8px", textTransform: "uppercase" }}>
            OUTBREAK
          </h1>
          <p style={{ color: "rgba(200,100,255,0.35)", fontSize: 11, marginBottom: 36, letterSpacing: 3 }}>
            SURVIVE THE HORDE
          </p>
          ...
          <div style={{ display: "flex", gap: 12, marginBottom: 28 }}>
            ...
            >
              ...
              <span style={{ color: "rgba(200,150,255,0.45)", fontSize: 11 }}>Create a room</span>
            </button>
            ...
            >
              ...
              <span style={{ color: "#a78bfa", fontSize: 14, fontWeight: 700, letterSpacing: 1 }}>SINGLE PLAYER</span>
              <span style={{ color: "rgba(167,139,250,0.45)", fontSize: 11 }}>Play alone</span>
            </button>
          </div>

          <p style={{ color: "rgba(200,100,255,0.25)", fontSize: 11, marginBottom: 20 }}>
            Team up with up to 4 players
          </p>

          {/* Join by code */}
          <div style={{
            background: "rgba(10,0,24,0.7)", border: "1px solid rgba(180,80,255,0.15)",
            borderRadius: 12, padding: "16px 18px",
          }}>
            <div style={{ fontSize: 11, color: "rgba(200,150,255,0.4)", letterSpacing: 1, marginBottom: 10 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <input
                ...
                maxLength={8}
                style={{
                  flex: 1, padding: "10px 12px", fontFamily: "monospace", fontWeight: 700, fontSize: 15,
                  background: "rgba(255,255,255,0.04)", border: "1px solid rgba(180,80,255,0.2)",
                  borderRadius: 8, color: "#fff", outline: "none", letterSpacing: 4, textAlign: "center",
                }}
              />
              <button
                onClick={joinRoom}
                disabled={joinInput.trim().length < 4 || joining}
                style={{
    ...
    <div style={bg}>
      ...
      <div style={{ position: "relative", zIndex: 1, textAlign: "center", width: "100%", maxWidth: 420, padding: "0 20px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 12, marginBottom: 6 }}>
          <span style={{ fontSize: 28 }}>💀</span>
          <h1 style={{ color: "#c084fc", fontSize: 22, fontWeight: 900, letterSpacing: 4, margin: 0, textTransform: "uppercase" }}>
            OUTBREAK
          </h1>
          {isFull && (
            <span style={{
              background: "rgba(239,68,68,0.2)", border: "1px solid rgba(239,68,68,0.4)",
              borderRadius: 6, padding: "2px 8px", fontSize: 11, color: "#f87171", fontWeight: 700,
            }}>FULL</span>
          )}
        </div>
        <div style={{ color: "rgba(200,150,255,0.45)", fontSize: 12, marginBottom: 24, letterSpacing: 2 }}>
          ROOM: {roomCode}
        </div>

        {/* Card */}
        <div style={{
          background: "rgba(10,0,24,0.85)", backdropFilter: "blur(12px)",
          border: "1px solid rgba(180,80,255,0.2)", borderRadius: 16,
          padding: 20, boxShadow: "0 8px 40px rgba(0,0,0,0.7)",
          marginBottom: 16,
        }}>
          {/* Players */}
            ...
            {displayPlayers.map((p) => (
              <div key={p.userId} style={{
                ...
                <div style={{
                  ...
                  background: "rgba(180,80,255,0.15)", overflow: "hidden", flexShrink: 0,
                ...
                <span style={{ color: "rgba(255,255,255,0.8)", fontSize: 13, flex: 1, textAlign: "left" }}>
                  @{p.username}
                </span>
                {p.isHost && (
                  <span style={{
                    background: "rgba(251,191,36,0.15)", border: "1px solid rgba(251,191,36,0.3)",
                    borderRadius: 5, padding: "2px 7px", fontSize: 10, color: "#fbbf24", fontWeight: 700,
                  }}>HOST</span>
                )}
              </div>
            ))}
          ...
          {/* Share hint */}
          <div style={{ fontSize: 10, color: "rgba(200,150,255,0.35)", letterSpacing: 0.5, marginBottom: 16 }}>
            Share code: <strong style={{ color: "rgba(200,150,255,0.6)", letterSpacing: 3 }}>{roomCode}</strong>
          </div>

          {/* Divider */}
          <div style={{ height: 1, background: "rgba(180,80,255,0.1)", marginBottom: 16 }} />
          ...
          {isHost ? (
            ...
            <div style={{ color: "rgba(200,150,255,0.4)", fontSize: 12, marginBottom: 10, padding: "10px 0" }}>
